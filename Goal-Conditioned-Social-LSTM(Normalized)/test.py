...
def main(dataset_name):
    ...
    # Use Predicted goal
    parser.add_argument('--goal', type=bool, default=False,
                        help='Use predicted goal or ground truth goal')

    # Parse the parameters
    sample_args = parser.parse_args()

    if sample_args.goal:
        print('Use predicted goal')
    else:
        print('Use true goal')
    ...
    log_test_file = open(os.path.join(log_test_directory_path, f'test_log_{int(sample_args.goal)}.txt'), 'w')
    ...
    with open(os.path.join(log_test_directory_path, f'test_ADE_FDE_{int(sample_args.goal)}.pkl'), 'wb') as f:
        pickle.dump(test_metrics, f)

    # Store the true and predicted trajectory
    with open(os.path.join(result_directory_path, f'test_true_predicted_trajectory_{int(sample_args.goal)}.pkl'), 'wb') as f:
...
def Test(epoch, test_dataset, net, saved_args, sample_args, dataset_name, test_metrics):
    ...
    open_estimation_file = open_directory + "goal_estimated_" + dataset_name + "_2.pkl"
    estimation_file = open(open_estimation_file,'rb')
    all_goal = pickle.load(estimation_file)
    estimated_goal = np.concatenate(all_goal['Predicted_Goal'])
    total_num_of_objs = 0
    ...
    for sequence in range(test_dataset.num_seq):
        ...
        if sample_args.goal:
            # Goal-conditioned Trajectory (Predicted Goal)
            num_of_objs = ori_seq.shape[0]
            traj_pred_e = estimated_goal[total_num_of_objs: num_of_objs + total_num_of_objs]
            traj_pred_e = np.expand_dims(traj_pred_e, 2)
            traj_pred_e = np.repeat(traj_pred_e, 20, axis=2)  # [pedestrian, 2, frame]
            total_num_of_objs += num_of_objs
            traj_goal_seq = traj_seq - traj_pred_e
        else:
            # Goal-conditioned Trajectory (Ground Truth Goal)
            traj_e = np.expand_dims(traj_e, 2)
            traj_e = np.repeat(traj_e, 20, axis=2)  # [pedestrian, 2, frame]
            traj_goal_seq = traj_seq - traj_e


        # Construct variables
        nodes = np.transpose(traj_goal_seq, (2, 0, 1)) #[frame, pedestrian, 2]
        ...
        nodes_ori = np.transpose(traj_seq, (2, 0, 1))
        nodes_ori = torch.from_numpy(nodes_ori).float()

        # Restore the predicted trajectory
        predicted_nodes[:sample_args.obs_length] = nodes_ori[:sample_args.obs_length]
        # if sample_args.goal:
        #     predicted_nodes = predicted_nodes + torch.from_numpy(np.transpose(traj_pred_e, (2, 0, 1))).float()
        # else:
        #     predicted_nodes = predicted_nodes + torch.from_numpy(np.transpose(traj_e, (2, 0, 1))).float()

        # Store true and predicted trajectory
        trajectory_result['True_trajectory'].append(nodes_ori.numpy())
        trajectory_result['Predicted_trajectory'].append(predicted_nodes.numpy())

        # Record the mean and final displacement error
        # a = torch.norm(nodes_ori[sample_args.obs_length:] - predicted_nodes[sample_args.obs_length:], dim=-1).mean()
        total_error_ = get_mean_error(predicted_nodes[sample_args.obs_length:].data,
                                      nodes_ori[sample_args.obs_length:].data,
...
if __name__ == '__main__':
    ...
    random.seed(20)
    # dataset = ['univ']
    dataset = ['eth','univ','zara1','zara2','hotel']
