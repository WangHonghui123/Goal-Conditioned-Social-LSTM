...
def train(epoch, train_dataset, net, optimizer, args, train_metrics, dataset_name):
    ...
    for batch_num, batch in enumerate(batch_set):
        ...
        for sequence in range(batch):
            ...
            traj_e = np.repeat(traj_e, 21, axis=2) #[pedestrian, 2, frame]
            ...
            ori_nodes = np.transpose(traj_seq, (2, 0, 1))
            ori_nodes = torch.from_numpy(ori_nodes).float()

            # Compute loss
            loss = Gaussian2DLikelihood(outputs, ori_nodes[1:], traj_node_present[1:], args.pred_length)
            loss_batch += loss
